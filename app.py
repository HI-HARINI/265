...
def make_grayscale(input_image):
    ...
    print(decode_array_to_image)
    converted_grayscale = cv2.cvtColor(decode_array_to_image,cv2.COLOR_BGR2GRAY)
    status,outputimage = cv2.imencode(".PNG",converted_grayscale)
    print(status)
    return outputimage
